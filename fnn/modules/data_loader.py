...
class Data_Loader:
    ...
    def __init__(
        ...
        distance=None,
        max_neigs=None,
        embedding_name="new_tuned_embedding_specific",
        k_folds=None,
        ...
        Parameters
        ----------
        traits: list or tuple, default: (0, 1, 2, 3, 4)
            OCEAN personality traits: O:0, C:1, E:2, A:3, N:4.
        distance: float, default: None
            In the case of coherence test, the distance to which perform the coherence test.
            Use None if you are not performing coherence test.
        ...
        embedding_name: string, default: 'new_tuned_embedding_specific'
        ...
        train_prop_holdout: float, default: None
            The proportion of training set on the known terms' set, in case of Holdout validation. Use None if you are
            not using K-fold cv.
        ...
        ----------
        self.data: list
            The list of preprocessing objects. In the i-th position is stored the preprocessing object associated with
            the i-th trait of traits' list.
        """
        ...
        if embedding_name != "glove":
            ...
            with open(os.path.join(embedding_path, "words.pickle"), "rb") as f:
                words = pickle.load(f)
            if embedding_name.endswith("specific"):
                self.data = []
                ...
                with open(
                        os.path.join(
                            embedding_path, "embedding.pickle"
                        ),
                        "rb",
                ) as f:
                    weights = pickle.load(f)[0].tolist()
